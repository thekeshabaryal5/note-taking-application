...
export const createNoteController = expressAsyncHandler(
  async (req, res, next) => {
    ...
    const [notes] = await connection.query( "SELECT * FROM notes WHERE user_id = ? AND LOWER(title) = LOWER(?)",[userId,title]);
    if(notes.length !== 0)
    {
    ...
    // Add note
    const[result] = await connection.query("Insert into notes (user_id,title,note,created_date,update_date) values (?,?,?,?,?)",[userId,title,note,date,date]);
...
export const updateNoteController = expressAsyncHandler(
  async (req, res, next) => {
    const userId = req.user.id;
    const noteId = req.params.id;
    const update_date =  new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Kathmandu" });
    ...
    const sql = `update notes set ${updates.join(", ")},update_date=? where user_id=? and note_id=?`;
